...
export const fetchSession = async (req, res, next) => {
	try {
		...
		//todo: create own org first and put id as orgId prop on account to be able to use _index for speed
		res.locals.org = await db.db().collection('orgs').findOne(currentOrgId ? { _id: ObjectId(currentOrgId) } : { owner: originalAccount._id });
