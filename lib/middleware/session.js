...
export const fetchSession = async (req, res, next) => {
	...
	}	// api impersonation already handled earlier by setting sessionUsername
	...
	if (sessionUsername) {
		...
		if (originalAccount) {
			...
			// operate as the original user by default if no impersonating org
			let accountToUse = originalAccount;

			// org impersonation if present in session
			const currentOrgId = req.session.currentOrg;
			const impersonateOwner = req.session.impersonateOwner;
			if (currentOrgId
				&& impersonateOwner
				&& originalAccount.onboarding !== true) { //no org usage until done/skipped onboarding
				console.log(originalAccount._id, 'current org', currentOrgId);
				console.log(originalAccount._id, 'impersonating owner', impersonateOwner);
				//check org membership
				const org = await db.db().collection('orgs').findOne({ _id: new ObjectId(currentOrgId) });
				if (org && org.members.includes(originalAccount._id)) {
					const ownerAccount = await db.db().collection('accounts').findOne({ _id: org.owner });
					if (ownerAccount) {
						accountToUse = ownerAccount;
					}
				} else {
			...
			const numCerts = await db.db().collection('certs')
				.countDocuments({ username: accountToUse._id });
			res.locals.user = {
				username: accountToUse._id,
				streamsId: accountToUse.streamsId,
				// email: accountToUse.email || null,
				...
				// billing: accountToUse.billing || null,
