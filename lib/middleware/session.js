...
async function getImpersonatingOrg(originalAccount, currentOrgId, impersonateOwner) {
	if (!currentOrgId || !impersonateOwner || originalAccount.onboarding === 7) {
		return { accountToUse: originalAccount, currentOrgId: originalAccount.orgId };
	}
...
export const fetchSession = async (req, res, next) => {
	try {
		...
		const currentOrgId = orgResult ? orgResult.currentOrgId : originalAccount.orgId;

		console.log(currentOrgId)

		res.locals.org = await db.db().collection('orgs').findOne({ _id: ObjectId(currentOrgId) });
		...
		res.locals.user = {
			...
			billing: accountToUse.billing,
			cc: req.headers['x-continent-code']?.toLowerCase() || 'global',
			currentOrg: currentOrgId || null,
		};

		res.locals.permissions = new Permission(); //no perms default, avoid nullpointer in places without calcPerms
