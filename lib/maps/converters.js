	...
	const hostsMap = await dataPlaneRetry('showRuntimeMap', { map: process.env.NEXT_PUBLIC_HOSTS_MAP_NAME })
		...
		.then(r => {
			return r.map(e => {
				...
				const { h: ipAndPort, cn, xp } = JSON.parse(e.value);
...
export async function handleMapValue(req, res) {
	...
	if ([
		...
		value = JSON.stringify({
			h: req.body.h,
			cn: req.body.geo,
			xp: req.body.xp === true ? true : false,
			c: req.body.c === true ? true : false,
		});
	} else if ([
		...
		value = JSON.stringify({
			...
			sl: req.body.sl === true ? true : false,
...
export async function handleMapKey(req, res) {
	...
	// validate hosts value ip:port and normalize req.body.h to host:port
	if (req.params.name === process.env.NEXT_PUBLIC_HOSTS_MAP_NAME) {
		...
		let parsedValue;
		try {
			parsedValue = new URL(`scheme://${req.body.h}`);
