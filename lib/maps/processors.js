...
export function parseJsonValues({ map, mapInfo, showValues: _showValues }) {
	map = map.map(a => {
		try {
	...
	return { map, mapInfo, showValues: _showValues };
}

// decodeURIComponent values for css strings (for valid map value)
export function decodeValues({ map, mapInfo, showValues: _showValues }) {
	map = map.map(a => {
		try {
...
export function imagesProcessor({ map, mapInfo, showValues: _showValues }) {
	map = map.filter(a => {
...
export function rewritesRedirects({ map, mapInfo, showValues: _showValues }) {
	map = map.filter(a => {
		const { pathname } = new URL(`https://${a.key}`);
		const isPowIconPath = pathname === `/${process.env.NEXT_PUBLIC_DOT_PATH}/pow-icon`;
		return !isPowIconPath;
...
export function showValues({ map, mapInfo, showValues: _showValues }) {
	showValues = true;
	return { map, mapInfo, showValues: _showValues };
}

// convert "ip|geo" into object
export function parseHostsValues({ map, mapInfo, showValues: _showValues }) {
	map = map.map(a => {
...
export function filterToUserDomains({ map, mapInfo, showValues: _showValues, res }) {
	map = map.filter(a => {
...
export function userValueSplitFilter({ map, mapInfo, showValues: _showValues, res }) {
	map = map
