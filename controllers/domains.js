...
export async function addDomain(req, res, next) {
	if (
		...
		req.body.domain.length === 0 ||
		req.body.domain.length > 200
	) {
	...
	try {
		...
		return dynamicResponse(req, res, 400, { error: 'Invalid domain' });
	}

	try {
		const parsed = psl.parse(domain);
		if (!parsed || !parsed.domain || !parsed.listed) {
			return dynamicResponse(req, res, 400, {
				error: 'Domain input is not a valid TLD',
			});
		}
		if (parsed.subdomain && !res.locals.user.domains.includes(parsed.domain)) {
			return dynamicResponse(req, res, 403, {
				error: 'Root domain must be added before subdomains',
			});
		}
		...
		const existing = await db
			...
			.findOne({
				$or: [
					{ domains: domain },
					{
						domains: new RegExp(`${parsed.domain}$`),
						_id: { $ne: res.locals.user.username },
					},
				],
			});
		if (existing) {
			return dynamicResponse(req, res, 400, {
				error: 'This domain is already in use or belongs to another user',
			});
		}
		await db
			.db()
		...
		// If not a subdomain, create the SOA and NS records for @
		if (!parsed.subdomain && getSoaTemplate && getNsTemplate) {
			// locked and templated SOA
			const soaRecords = JSON.parse(JSON.stringify(getSoaTemplate()));
			soaRecords[0].MBox = `root.${domain}.`;
			soaRecords[0].l = true;
			soaRecords[0].t = true;

			// locked and templated ND
	...
	return dynamicResponse(req, res, 302, {
		redirect: req.body.onboarding ? '/onboarding' : '/domains',
	});
}
...
export async function deleteDomain(req, res) {
	...
	// check if in use in some map
	const [existingHost, existingMaintenance, existingRewrite, existingDdos] =
		await Promise.all([
			res.locals
				.dataPlaneRetry('showRuntimeMap', {
					map: process.env.NEXT_PUBLIC_HOSTS_MAP_NAME,
				})
				.then((r) => r.data)
				.then((map) => map.some((e) => e.key === domain)),
			res.locals
				.dataPlaneRetry('showRuntimeMap', {
					map: process.env.NEXT_PUBLIC_MAINTENANCE_MAP_NAME,
				})
				.then((r) => r.data)
				.then((map) => map.some((e) => e.key === domain)),
			res.locals
				.dataPlaneRetry('showRuntimeMap', {
					map: process.env.NEXT_PUBLIC_REWRITE_MAP_NAME,
				})
				.then((r) => r.data)
				.then((map) => map.some((e) => e.key === domain)),
			res.locals
				.dataPlaneRetry('showRuntimeMap', {
					map: process.env.NEXT_PUBLIC_DDOS_MAP_NAME,
				})
				.then((r) => r.data)
				.then((map) =>
					map.some((e) => {
						const { hostname } = new URL(`https://${e.key}`);
	...
	if (existingHost || existingMaintenance || existingRewrite || existingDdos) {
		return dynamicResponse(req, res, 400, {
			error:
				'Cannot remove domain while still in use. Remove it from backends/maintenance/rewrites/protection first.',
		});
	}
